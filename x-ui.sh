...
install() {
    cd ~
    url="https://github.com/parentalclash/x-ui-freebsd/releases/download/f0.3.2.1/x-ui-freebsd-amd64.tar.gz"
    ...
    # 解压并安装
    tar -zxvf x-ui-${release}-${arch}.tar.gz -C ~/
    if [[ $? == 0 ]]; then
        echo && echo -n -e "请输入面板访问端口 [默认:54321]: " && read custom_port
        echo && echo -n -e "请输入用户名 [默认:admin]: " && read custom_user
        echo && echo -n -e "请输入密码 [默认:admin]: " && read custom_pass
        echo && echo -n -e "请输入流量监听端口 [默认:9999]: " && read custom_traffic_port
        
        [ -z "$custom_port" ] && custom_port="54321"
        [ -z "$custom_user" ] && custom_user="admin"
        [ -z "$custom_pass" ] && custom_pass="admin"
        [ -z "$custom_traffic_port" ] && custom_traffic_port="9999"
        
        ~/x-ui/x-ui setting -port $custom_port -username $custom_user -password $custom_pass -trafficport $custom_traffic_port
...
reset_user() {
    ...
    ~/x-ui/x-ui setting -username $custom_user -password $custom_pass
    echo -e "用户名和密码已重置为 ${green}${custom_user}/${custom_pass}${plain}，现在请重启面板"
    confirm_restart
}
...
start() {
    ...
    if [[ $? == 0 ]]; then
        ...
        if [[ $? == 0 ]]; then
            ...
            LOGE "面板启动失败，可能是因为启动时间超过了预期，请稍后查看日志信息"
        fi
    fi
...
stop() {
    ...
    if [[ $? == 1 ]]; then
        ...
        if [[ $? == 1 ]]; then
            ...
            LOGE "面板停止失败，可能是因为停止时间超过了预期，请稍后查看日志信息"
        fi
    fi
...
restart() {
    ...
    if [[ $? == 0 ]]; then
        ...
        LOGE "面板重启失败，可能是因为启动时间超过了预期，请稍后查看日志信息"
    fi
    if [[ $# == 0 ]]; then
        before_show_menu
    fi
